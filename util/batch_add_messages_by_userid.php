...
if (($handle = fopen($file, "r")) !== false) {
    ...
    while (($data = fgetcsv($handle, 1000, ",")) !== false) {
        ...
        if ($notify->idnumber == "#N/A") {
            echo('No record for user with idnumber: '.$notify->idnumber."\n");
            continue;
        }

        $user  = $DB->get_record('user', array('idnumber' => $notify->idnumber));
        if (!$user) {
            echo('No record for user with idnumber: '.$notify->idnumber."\n");
            continue;
        }
        ...
        if (!$result) {
            echo('Zero returned for id inserting '.$notify->idnumber."\n");
            continue;
        }
        $numadded++;
    }
